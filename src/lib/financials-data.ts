...
export type ReceivedMonth = {
  ...
  year: number;
  month: number; // 0-11
  received: number; // sum(total_cost)
  hospitalShare: number; // sum(hospital_share)
  count: number;
};
...
export type FinancialsData = {
  ...
  receivedByMonth: ReceivedMonth[]; // by payment_received_at, sorted NEWEST first
};
...
function parse(d: Record<string, unknown>): FinancialsData {
  ...
  const receivedByMonth = ((d.received_by_month ?? []) as { y: number; m: number; received: number; hospital_share: number; count: number }[])
    .map((r) => ({ key: monthKey(r.y, r.m), year: r.y, month: r.m - 1, received: num(r.received), hospitalShare: num(r.hospital_share), count: num(r.count) }))
...
async function fromRows(
  ...
  return {
    ...
    receivedByMonth: Array.from(recvByMonth.values()).sort((a, b) => b.key.localeCompare(a.key)),
  };
}
